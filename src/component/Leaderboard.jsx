...
const LeaderboardPanel = () => {
    ...
    // Yahan hum available roles ki list banate hain, jo chune gaye department par nirbhar karti hai.
    const availableRoles = useMemo(() => {
        let data = [...dataSource];
        if (departmentFilter !== "All") {
            data = data.filter((user) => user.department === departmentFilter);
        }
        return Array.from(new Set(data.map((user) => user.role)));
    }, [departmentFilter, dataSource]);

    // Jab bhi department badalta hai, hum role filter ko 'All' par reset kar dete hain.
    // Isse galat filter combination se bacha ja sakta hai.
    useEffect(() => {
        setRoleFilter("All");
    }, [departmentFilter]);

    // Yahan asli filtering ka kaam hota hai.
    // Yeh department, role, aur search query ke hisaab se data ko filter karta hai.
    const filteredData = useMemo(() => {
        let data = [...dataSource];
        // Department ke hisaab se filter
        if (departmentFilter !== "All") {
            data = data.filter((user) => user.department === departmentFilter);
        }
        ...
        // Search query ke hisaab se filter
        if (searchQuery) {
            data = data.filter((user) => user.name.toLowerCase().includes(searchQuery.toLowerCase()));
        }

        // Points ke aadhar par sort karke rank assign karna.
    ...
    return (
        ...
        <div className="min-h-screen flex flex-col items-center p-0 md:p-8 font-sans relative" style={{
            ...
            {/* *** FIX: Added relative and z-20 to ensure this div is on top of other content *** */}
            <div className={`w-full bg-gray-900/90 shadow-xl overflow-hidden transition-all duration-300 ease-in-out relative z-20 ${showFilters ? 'max-h-96 opacity-100 py-4 border-b border-gray-700' : 'max-h-0 opacity-0 py-0'}`}>
                <div className="max-w-6xl mx-auto px-4 flex flex-wrap gap-4 items-center justify-center lg:justify-start">
                    ...
                    <button
                        ...
                        className="p-2 bg-red-600 text-white rounded-lg hover:bg-red-500 transition duration-150 ease-in-out text-sm font-semibold"
                    >
                        Reset Filters
                    </button>
                </div>
            ...
            <div className="w-full bg-gray-900/80 p-4 shadow-inner border-t border-gray-800 z-10">
                <div className="flex justify-center max-w-6xl mx-auto space-x-3">
                    {["Today", "Weekly", "Monthly"].map(tab => (
                        <button
                            key={tab}
                            className={`py-2 px-4 sm:px-6 rounded-full font-bold transition duration-300 text-sm md:text-base ${activeTab === tab
            ...
            <div className="w-full max-w-6xl mx-auto p-4 flex flex-col items-center relative z-10">
                ...
                <div className="w-full bg-gray-900/90 p-6 rounded-2xl shadow-2xl transition duration-300 transform border border-blue-900/50 mt-6">
                    ...
                    <div className="w-full text-center mb-6 p-3 bg-gray-800 rounded-lg border border-gray-700">
                        <p className="text-sm text-gray-400">
                            Abhi filter kiya gaya hai:
                            <span className="font-bold text-blue-400 mx-2">
                                {departmentFilter === 'All' ? 'Sabhi Departments' : departmentFilter}
                            </span>
                            |
                            <span className="font-bold text-purple-400 mx-2">
                                {roleFilter === 'All' ? 'Sabhi Roles' : roleFilter}
                            </span>
                        </p>
                    </div>
                    ...
                    {hasResults ? (
                        <>
                            ...
                            <div className="flex justify-around items-end h-[300px] sm:h-[350px] w-full max-w-lg mx-auto mb-10">
                                <PodiumItem user={rank2} rank={2} currentUserName={CURRENT_USER_NAME} animate={animatePodium} delayIndex={1} />
                                <PodiumItem user={rank1} rank={1} currentUserName={CURRENT_USER_NAME} animate={animatePodium} delayIndex={0} />
                                <PodiumItem user={rank3} rank={3} currentUserName={CURRENT_USER_NAME} animate={animatePodium} delayIndex={2} />
                            </div>

                            {/* --- List Section (Ranks 4+) --- */}
                            <div className="space-y-2 max-w-4xl mx-auto">
                                {visibleList.map((user) => <LeaderboardItem key={user.name} user={user} />)}
                            </div>

                            {/* --- Separator and Current User --- */}
                            {isCurrentUserOutsideVisibleList && (
                                <>
                                    <div className="text-center my-4 font-bold text-gray-500 tracking-widest">...</div>
                                    <LeaderboardItem user={currentUser} />
                                </>
                            )}
                            {hasMoreUsers && !isCurrentUserOutsideVisibleList && (
                        ...
                        <div className="text-center py-20">
                            ...
                            <p className="text-gray-500">Try adjusting your filters to find contributors.</p>
                        </div>
                    )}
                </div>
            </div>
